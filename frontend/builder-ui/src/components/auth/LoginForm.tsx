...
const OAuthButton: React.FC<OAuthButtonProps> = ({ provider }) => {
  const { getOAuth2Url } = useAuth();
  const [isLoading, setIsLoading] = useState(false);

  const handleOAuthLogin = async () => {
    setIsLoading(true);
    try {
      const authData = await getOAuth2Url(provider);
      if (authData) {
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className={`w-full inline-flex justify-center py-2 px-4 border ${config.borderColor} ${config.bgColor} ${config.textColor} text-sm font-medium ${config.hoverBg} rounded-md shadow-sm disabled:opacity-50 disabled:cursor-not-allowed`}
    >
      {isLoading ? (
